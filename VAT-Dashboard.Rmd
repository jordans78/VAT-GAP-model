...
t_df1g<-t_df1%>%
  dplyr:: group_by(deciles)%>%
  dplyr:: summarise(GroupSum = sum(value  , na.rm = T))%>%
  select(deciles,GroupSum)



t_df1g_final<-left_join(t_df1,t_df1g,by = c("deciles"))


t_df1g_final1<-group_by(t_df1g_final,deciles) %>% mutate(Pct = value/GroupSum)


t_df1g_final1$color <- factor(t_df1g_final1$variable, labels = c("royalblue", "cyan", "darkorange", "red", "brown",
                                                 "chartreuse","orange","purple", "gold","tomato",
                                                 "darkturquoise", "forestgreen"))


#CHART
Expenditure_chart_pct <- plot_ly(t_df1g_final1, x = ~deciles, y = ~Pct,
                                 type = 'bar',
                                 marker = list(color = ~color), name = ~variable) %>%
  layout(title = " ",
         xaxis = list(title = ''),
...
### Structure of VAT revenues according to the purpose of consumption, per households decile groups
