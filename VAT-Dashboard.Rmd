...
Compsition_TaxSSC_Revenues <- plot_ly(t_df_1_pct2, x = ~Years, y = ~value*1000000,
                                      type = 'bar',
...
VAT_FORECAST_HISTORIC<-VAT_FORECAST%>%
 ...
 VAT_FORECAST_HISTORIC$Nominal_VAT_NET<- (VAT_FORECAST_HISTORIC$Nominal_VAT_NET)*1000000
                                ...
                                fig <- fig %>% add_trace(x = ~Year, y = ~share_gdp*100, type = 'scatter', mode = 'lines', name = 'Share of VAT in GDP', yaxis = 'y2', marker = list(color = 'red'),
                                                         line = list(dash = "dash",width=5)
                                                         
                                                         )
                                fig <- fig %>% layout(title = ' ',
                                                      xaxis = list(title = "",font = t_8),
                                                      yaxis = list(side = 'left', title = '', font = t_8,showgrid = FALSE, zeroline = FALSE),
                                                      yaxis2 = list(side = 'right', overlaying = "y", title = '',font = t_8, showgrid = FALSE, zeroline = FALSE))
...
### Structure of tax and SSC in percentage
...
t_df_pct1$Years<- factor(t_df_pct1$Years)
t_df_pct2<-melt(t_df_pct1, id.vars = c("Years"))
t_df_pct2$color <- factor(t_df_pct2$variable, labels = c("orange", "red"))
...
t_df_1_pct1$Years<- factor(t_df_1_pct1$Years)
t_df_1_pct2<-melt(t_df_1_pct1, id.vars = c("Years"))
t_df_1_pct2$color <- factor(t_df_1_pct2$variable, labels = c("orange", "red"))
...
df<-df%>%
  dplyr::rename(c("CPA Division"= "PRODUCT_INDUSTRY_CODE",
                  "Description"= "PRODUCT_INDUSTRY_NAME",
                  "Standard Rate"="Standard_VAT_Rate",
                  "Preferential rate"="Preferential_VAT_Rate",
                  "Exempted Share"="Simulated_Policy_Exempt",
...
### Comparison VAT revenues (in LCU)
...
### Effective VAT rates accross CPA division
...
fig <- fig %>% layout(title = ' ',
                      xaxis = list(title = 'Expenditures in LCU million'),
...
vat_effective<-hbs%>%
  ...
  dplyr::filter(total_consumption<=6500000)
vat_effective$total_consumption<-NULL

vat_effective_hbs<-cbind(vat_effective_bu,vat_effective)%>%
  select(total_consumption,effective_vat_before_reform,effective_vat_after_reform)


fit <- sm.regression(vat_effective_hbs$total_consumption, vat_effective_hbs$effective_vat_before_reform, display="none")


fit1 <- sm.regression(vat_effective_hbs$total_consumption, vat_effective_hbs$effective_vat_after_reform, display="none")

data_merged_plot<-data.frame(
   total_consumption=fit$eval.points,
   effective_vat_before_reform=fit$estimate,
   effective_vat_after_reform=fit1$estimate)
  


fig <- plot_ly(data_merged_plot, x = ~total_consumption, y = ~effective_vat_before_reform, name = "Before reform", type = 'scatter', mode = 'lines',
               line = list(dash = "solid",width=5))
fig <- fig %>% add_trace(y = ~effective_vat_after_reform, name = "After reform", line = list(dash = "dash",width=5))%>%
  layout(
    xaxis = list(title = 'Expenditures in LCU million'),
    yaxis = list(title = ''),
...
plt <- plot_ly(vat_effective_final) %>% 
  add_trace(x = ~deciles, y = ~effective_vat_before_reform, type = 'bar', name = 'With current VAT rates') %>% 
  add_trace(x = ~deciles, y = ~effective_vat_after_reform, type = 'bar', name = 'With standard VAT rates') %>% 
    layout(
       xaxis = list(title = 'Deciles',font = t_8), 
    yaxis = list(title = ''),
    legend = list(x = 0.01, y = 0.99),
    barmode = 'group'
     )
...
dat_r<-hbs_bu%>%
  dplyr::select(deciles,"Food and Non-Alcoholic Beverages",
...
Expenditure_chart <- plot_ly(t_df1, x = ~deciles, y = ~value,
                                   type = 'bar',
                                   marker = list(color = ~color), name = ~variable) %>%
        #layout(yaxis = list(title = 'Millions denars'), barmode = 'stack')
        #layout(title = " ",yaxis = list(title = 'Millions denars'), barmode = 'stack')
...
### Used assets according to the purpose of consumption, per household in percentage

```{r,eval=FALSE}

#t_df2 <- data.frame((DATA_PLOT7[,-1]))
      # t_df2_pct <- t_df2
      # t_df2_pct[,1:(ncol(DATA_PLOT7)-1)] <- t_df2_pct[,1:(ncol(DATA_PLOT7)-1)]/rowSums(t_df2_pct[,1:(ncol(DATA_PLOT7)-1)])
      # 
      # DECILE_THRESHOLD<-c("First","Second","Third","Fourth","Fifth","Sixth","Seventh","Eight","Ninth","Tenth")
      # t_df2_pct1<-cbind(DECILE_THRESHOLD ,t_df2_pct)
      # t_df2_pct1$DECILE_THRESHOLD<- factor(t_df2_pct1$DECILE_THRESHOLD, levels = c("First","Second","Third","Fourth","Fifth","Sixth","Seventh","Eight","Ninth","Tenth"))
      # 
      
      t_df1$value<-as.factor(t_df1$value)

      t_df1$Pct<-prop.table(t_df1$value)
      
      t_df2_pct2<-melt(t_df2_pct1, id.vars = c("DECILE_THRESHOLD"))
      t_df2_pct2$color <- factor(t_df2_pct2$variable, labels = c("royalblue", "cyan", "darkorange", "red", "brown",
                                                               "chartreuse","orange","purple", "gold","tomato",
                                                               "darkturquoise", "forestgreen", "magenta"))
      #CHART
      Expenditure_chart_pct <- plot_ly(t_df2_pct2, x = ~DECILE_THRESHOLD, y = ~value,
                                       type = 'bar',
                                       marker = list(color = ~color), name = ~variable) %>%
        layout(title = " ",
               yaxis = list(title = 'Percentage'), barmode = 'stack')
      Expenditure_chart_pct<-ggplotly(Expenditure_chart_pct)
      Expenditure_chart_pct
...
#t_df1$colr <- gradient("viridis", steps = 120)

# t_df1$colr <- gradient(c("royalblue", "cyan", "darkorange", "red", "brown",
#   "chartreuse","orange","purple", "gold","tomato",
#   "darkturquoise", "forestgreen"),steps = 120)


t_df1$colr <- factor(t_df1$variable, labels = c("royalblue", "cyan", "darkorange", "red", "brown",
                                                 "chartreuse","orange","purple", "gold","tomato",
                                                 "darkturquoise", "forestgreen"))

t_df1<-plot_ly(data = t_df1, labels = ~variable, values = ~value, type = "pie",
...
#   add_trace(x = ~deciles+0.4, y = ~value, type = 'bar', marker = list(color = ~color), showlegend = FALSE) %>%
#   layout(
#     xaxis = list(title = '',font = t_16),
#     yaxis = list(title = ''),
...
t_df4 <- pivot_longer(t_df3, cols = c(value.x,value ),
                      names_to = "group") %>% 
  mutate(group = ifelse(group == "value", " After reform",
                        "Before reform"))
...
fig <- plot_ly(VAT_FORECAST, x = ~Year , y = ~Nominal_VAT_NET, name = "Before reform", type = 'scatter', mode = 'lines',
               line = list(dash = "solid",width=5))
fig <- fig %>% add_trace(y = ~New_Simulation_VAT, name = "After reform", line = list(dash = "dash",width=5))%>%
  layout(
...
VAT_FORECAST$Tax_expenditures<-VAT_FORECAST$Tax_expenditures*1000000

plt <- plot_ly(VAT_FORECAST) %>%
  add_trace(x = ~Year , y = ~Tax_expenditures, type = 'bar', name = 'With current VAT rates') %>%
    layout(
       xaxis = list(title = '',font = t_8),
    yaxis = list(title = ''),
    legend = list(x = 0.01, y = 0.99),
...
#VAT_FORECAST$Year<-as.factor(VAT_FORECAST$Year)


# fig <- plot_ly(
#   VAT_FORECAST, name = "Tax_expenditures", type = "waterfall", #measure = ~measure,
#   x = ~Year, textposition = "outside", y= ~Tax_expenditures, #text =~text,
