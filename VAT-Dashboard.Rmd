...
Simulation<- read_excel("simulation.xlsx",sheet="hbs")
...
plt <- plot_ly(t_df3) %>%
  add_trace(x = ~DECILE_THRESHOLD, y = ~value.x, type = 'bar',name = 'Before reform',marker = list(color = ~color.x), name = ~variable.x) %>%
  add_trace(x = ~DECILE_THRESHOLD, y = ~value, type = 'bar',name = 'After reform', marker = list(color = ~color), name = ~variable) %>%
  layout(
    xaxis = list(title = '',font = t_8),
    yaxis = list(title = ''),
    legend = list(x = 0.01, y = 0.99,font = t_10),
